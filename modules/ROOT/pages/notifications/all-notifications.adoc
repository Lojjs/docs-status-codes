...
The variable `{ <<variable>> }` in the subquery uses the same name as a variable from the outer query. Use `CALL ({ <<variable>> })` to import the one from the outer scope unless you want it to be a new variable.
|Classification
m|GENERIC
|SeverityLevel
m|INFORMATION
